                ...
                try {
                    ...
                    if ($locationsResponse->successful()) {
                        ...
                        if ($selectedLocationId) {
                            ...
                            // the "Select Sub-Account" modal (OAuth state captured on
                            // /landing). Register the provider for ONLY that location, even
                            // if oauth/installedLocations still reports it as
                            // isInstalled:false (GHL marks it installed a moment later).
                            $selectedEntry = null;
                            ...
                            Log::info('🎯 [BULK] Targeted install - registering provider ONLY for the user-selected location', [
                                'selectedLocationId' => $selectedLocationId,
                                'note' => 'Selected via the Select Sub-Account modal; ignoring isInstalled flag for this location'
                            ]);
                        } else {
                            // GENERAL INSTALL (e.g. "select all"/agency): register for every
                            // location GHL reports as isInstalled: true.
    ...
    public function connectOrDisconnect(Request $request)
    {
        ...
        if ($action === 'connect') {
            ...
            $payload = [
                ...
                'description' => 'Supports Visa, MasterCard, and KNET payments via Tap Create Charge API. Payments are processed directly through the Tap hosted payment page with full 3D Secure support. Note: PayPal is not supported.',
                'paymentsUrl' => 'https://dashboard.mediasolution.io/charge',
                'queryUrl'    => 'https://dashboard.mediasolution.io/api/payment/query',
                'imageUrl'    => 'https://msgsndr-private.storage.googleapis.com/marketplace/apps/68323dc0642d285465c0b85a/11524e13-1e69-41f4-a378-54a4c8e8931a.jpg',
