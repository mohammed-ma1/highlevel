...
class ClientIntegrationController extends Controller
{
    ...
    public function handleTapWebhook(Request $request)
    {
        try {
            ...
            if (!$chargeId || !$status) {
                Log::warning('Invalid webhook data', ['webhookData' => $webhookData]);
                return response()->json(['status' => 'error', 'message' => 'Invalid webhook data'], 400);
            }

            Log::info('Processing Tap webhook', [
                'chargeId' => $chargeId,
                'status' => $status,
                'amount' => $amount,
                'currency' => $currency
            ...
            switch ($status) {
                case 'CAPTURED':
                    $this->handleTapPaymentCaptured($chargeId, $webhookData);
                    break;
                    
                case 'FAILED':
                case 'CANCELLED':
                case 'DECLINED':
                    $this->handlePaymentFailed($chargeId, $webhookData);
                    break;
                    
                default:
                    Log::info('Unhandled webhook status', ['status' => $status, 'chargeId' => $chargeId]);
            }
    ...
    /**
     * Handle successful payment capture from Tap webhook
     */
    private function handleTapPaymentCaptured($chargeId, $webhookData)
    {
        try {
            $reference = $webhookData['reference'] ?? [];
            $merchant = $webhookData['merchant'] ?? [];
            $transactionId = $reference['transaction'] ?? null;
            $orderId = $reference['order'] ?? null;
            $locationId = $merchant['id'] ?? null;
            $amount = $webhookData['amount'] ?? 0;
            $currency = $webhookData['currency'] ?? 'USD';
            ...
            if (!$transactionId || !$locationId) {
                Log::warning('Missing required data for GHL webhook', [
                    'transactionId' => $transactionId,
                    'locationId' => $locationId
                ]);
                return;
            ...
            Log::error('Failed to handle payment capture', [
                'error' => $e->getMessage(),
                'chargeId' => $chargeId
            ]);
        }
     ...
     */
    private function handlePaymentFailed($chargeId, $webhookData)
    {
    ...
    /**
     * Send webhook to GHL backend
     */
    private function sendGHLWebhook($data)
    {
        try {
            $webhookUrl = 'https://backend.leadconnectorhq.com/payments/custom-provider/webhook';
            
            Log::info('Sending webhook to GHL', [
                'url' => $webhookUrl,
                'data' => $data
            ]);
            ...
            $response = Http::timeout(30)
                ->withHeaders([
                    ...
                    'User-Agent' => 'Tap-Payment-Integration/1.0'
                ])
                ->post($webhookUrl, $data);

            if ($response->successful()) {
                Log::info('GHL webhook sent successfully', [
            ...
            Log::error('Failed to send GHL webhook', [
                'error' => $e->getMessage(),
                'data' => $data
            ]);
        }
    ...
    public function createTapCharge(Request $request)
    {
        try {
            ...
            $tapData = [
                ...
                'redirect' => $data['redirect'] ?? ['url' => config('app.url') . '/payment/success?charge_id=']
            ];
