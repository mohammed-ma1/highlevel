...
class ClientIntegrationController extends Controller
{
    ...
    public function createTapCharge(Request $request)
    {
        try {
            ...
            // Extract source information - handle both object format and direct id
            $sourceId = null;
            if (isset($data['source'])) {
                if (is_array($data['source']) && isset($data['source']['id'])) {
            ...
            // Check if this is a Knet payment
            // Knet payments use either 'src_kw.knet' or a token ID generated by the SDK for Knet
            // We detect Knet by checking for the explicit source ID or if explicitly indicated in the request
            $isKnetPayment = false;
            if ($sourceId) {
                // Explicit Knet source ID
                if ($sourceId === 'src_kw.knet') {
                    $isKnetPayment = true;
                }
                // Check if request explicitly indicates Knet payment
                elseif (isset($data['payment_method']) && strtolower($data['payment_method']) === 'knet') {
                    $isKnetPayment = true;
                }
                // If a token is provided and it's for Knet (check metadata or explicit flag)
                elseif (strpos($sourceId, 'tok_') === 0 && isset($data['is_knet']) && $data['is_knet'] === true) {
                    $isKnetPayment = true;
                }
            }
            
            // For Knet payments, ensure customer_initiated and threeDSSecure are true
            $customerInitiated = $isKnetPayment ? true : ($data['customer_initiated'] ?? true);
            $threeDSecure = $isKnetPayment ? true : ($data['threeDSecure'] ?? true);
